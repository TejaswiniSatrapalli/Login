...
public class ExchangeRateServiceImpl implements ExchangeRateService {
	...
	public ExchangeRateDto saveExchangeRate(ExchangeRateDto exchangeRateDto) {
		...
		ExchangeRate exRate =new ExchangeRate();
         ...
         exRate.setOurFee(exchangeRateDto.getOurFee());
         exRate.setBankTransferFee(exchangeRateDto.getBankTransferFee());
	...
	@Override
	public Double convertAmount(Double amount, String fromCurrency, String toCurrency, Double ourFee, Double bankTransferFee) {
	    ExchangeRate exchangeRate = exchangeRateRepository.findByFromCurrencyAndToCurrency(fromCurrency, toCurrency);
	    if (exchangeRate == null) {
	        throw new ExchangeRateNotFoundException("Exchange Rate Not found " + fromCurrency + "/" + toCurrency);
	    }

	    Double rate = exchangeRate.getRate();
	    Double convertedAmount = amount - (amount * ourFee) - bankTransferFee;
	    Double recipientAmount = rate * convertedAmount;
...
//		Double rate = exchangeRate.getRate();
//
//		return rate * (amount-(amount*ourFee)-bankTransferFee);
//	}
//
